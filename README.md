...
#  Choix finaux :
#    - PostgreSQL Option A (bases séparées, instance unique)
#    - Keycloak : KC_DB + --import-realm + export automatique
# ══════════════════════════════════════════════════════════════
...
services:
  ...
  # ════════════════════════════════
  # 1. POSTGRESQL — Option A
  #    Une instance, plusieurs bases
  #    init.sql crée toutes les bases
  # ════════════════════════════════
  ...
  postgres:
    ...
    environment:
      ...
      POSTGRES_DB: keycloak_db          # base principale (Keycloak)
    ports:
      - "5432:5432"
    volumes:
  ...
  # ════════════════════════════════
  # 5. KEYCLOAK IAM
  #    - KC_DB : persistance PostgreSQL
  #    - --import-realm : init automatique
  #    - export-realm.sh : export avant down -v
  # ════════════════════════════════
  ...
  ms-collecte:
    ...
    environment:
      ...
      # base dédiée ms-collecte
      SPRING_DATASOURCE_URL: jdbc:postgresql://postgres:5432/collecte_db
  ...
  ms-ml:
    ...
    environment:
      ...
      INFLUXDB_BUCKET: ${INFLUXDB_BUCKET:-fleet-metrics}
      # base dédiée ms-ml
      POSTGRES_URL: postgresql://${POSTGRES_USER:-fleet_user}:${POSTGRES_PASSWORD:-fleet_pass}@postgres:5432/ml_db
    depends_on:
      kafka:
        condition: service_healthy
      influxdb:
